...
import 'program_details_screen.dart'; // Add this import
...
class _MainscreenState extends State<Mainscreen> {
  ...
  void initState() {
    ...
    // Seed demo content
    _seedDemoData();
    fetchFeaturedCourses(); // Add this line
  }
  ...
  Future<void> fetchFeaturedCourses() async {
    ...
    try {
      final response = await http.get(
        Uri.parse('https://68f92c82deff18f212b8dd59.mockapi.io/api/data/Courses?page=1&limit=5'),
      );

      if (response.statusCode == 200) {
        final List<dynamic> data = json.decode(response.body);
        setState(() {
          featuredCourses = data.map((json) => Course.fromJson(json)).toList();
          isLoadingFeatured = false;
        });
      } else {
        throw Exception('Failed to load featured courses');
      }
    } catch (e) {
      setState(() {
        isLoadingFeatured = false;
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Error loading featured courses: $e')),
      );
    }
  }
            ...
            Expanded(
              child: isLoadingFeatured
                ...
                : ListView.builder(
                    ...
                    itemBuilder: (ctx, i) {
                      final course = featuredCourses[i];
                      return Card(
